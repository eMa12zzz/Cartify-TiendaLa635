...
const ConfirmActionModal = ({ isOpen, onClose, onConfirm, product, actionType, brands = [], suppliers = [], categories = [] }) => {
  ...
  const imagePreview = isDelete 
    ...
    : (product?.selectedImage 
        ? URL.createObjectURL(product?.selectedImage) 
        : (Array.isArray(product?.originalImage) ? product.originalImage[0] : product?.originalImage)
      );

  // Use original maxQuantity if editing, otherwise use current stock as max, fallback to 100
  const maxStock = displayData?.maxQuantity || product?.originalProduct?.maxQuantity || displayData?.stock || 100;
        ...
        <div className="flex w-full bg-white">
          <div className="w-1/3 border-r border-gray-200 p-6 flex flex-col items-center">
            <div className="w-full text-left mb-4">
              ...
              <h3 className="text-xl font-bold text-gray-900 leading-tight">{displayData?.name}</h3>
            </div>
            
            <div className="flex-1 flex items-center justify-center py-4 w-full bg-gray-50 border border-gray-100 rounded-xl mb-4 overflow-hidden h-48">
              {imagePreview ? (
                <img src={imagePreview} alt={displayData?.name} className="w-full h-full object-contain" />
              ) : (
                <p className="text-sm text-gray-400">Sin imagen</p>
              )}
            </div>
            
            <div className="text-xs text-gray-600 text-center border-t border-gray-200 pt-3 w-full font-medium">
              Costo: ${Number(displayData?.priceCost || 0).toFixed(2)} | Precio: ${Number(displayData?.salePrice || 0).toFixed(2)}
            </div>
          </div>

          <div className="w-2/3 p-6 flex flex-col relative bg-white">
            ...
            <div className="mb-8">
              <p className="text-sm font-medium text-gray-800">
                Fecha de expiración: {displayData?.expirationDate ? new Date(displayData.expirationDate).toLocaleDateString() : 'N/A'}
              </p>
            </div>
