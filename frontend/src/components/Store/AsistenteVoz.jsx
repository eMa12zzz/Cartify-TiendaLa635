 ...
 * Solo pinta: toda la lógica vive en useVoiceAssistant. Recibe el carrito y sus
 * funciones por props para compartir el estado con la tienda.
 *
 * Diseño para la tercera edad: fondo oscuro de alto contraste, letra grande,
 * micrófono enorme, y escucha continua (manos libres) — el cliente solo habla.
 */
const AsistenteVoz = ({
  onClose, productos, carrito, totalCarrito,
  agregarAlCarrito, eliminarDelCarrito, actualizarCantidad, limpiarCarrito,
}) => {
  const { activo, escuchando, transcripcion, respuesta, iniciar, detener, hablar, soportado } =
    useVoiceAssistant({
    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'rgba(17,17,17,0.95)' }}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 text-center max-w-3xl mx-auto w-full">
        ...
        <p className="text-lg md:text-xl text-gray-300 mb-10">
          Toca el micrófono y di qué quieres. Ejemplo: <span className="text-white font-semibold">"quiero una manzana"</span>
        </p>

        {/* Micrófono gigante (enciende/apaga la escucha manos libres) */}
        <button
        ...
        <p className="mt-5 text-base md:text-lg font-semibold" style={{ color: estadoColor }}>{estadoTexto}</p>

        {activo && (
          <p className="mt-1 text-sm text-gray-400">Manos libres activo — solo habla. Toca de nuevo para pausar.</p>
        )}
        ...
        {!soportado && (
          <p className="mt-6 text-sm text-red-300">Tu navegador no soporta reconocimiento de voz. Usa Chrome o Edge.</p>
        )}
      </div>
