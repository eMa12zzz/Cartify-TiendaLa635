...
const NavBtn = styled.button`background: white; color: #333; border: 1.5px solid #e0e0e0; cursor: pointer; display: flex; align-items: center; gap: 6px; padding: 8px 14px; border-radius: 30px; font-size: 13px; font-weight: 600; transition: all 0.2s; &:hover { border-color: ${BROWN}; color: ${BROWN}; }`;
...
const Tab = styled.button`display: flex; align-items: center; gap: 8px; padding: 12px 18px; border-radius: 12px; border: 1.5px solid ${p => (p.$active ? BROWN : '#e0e0e0')}; background: ${p => (p.$active ? '#f5ede4' : 'white')}; color: ${p => (p.$active ? BROWN_DARK : '#555')}; font-size: 14px; font-weight: 700; cursor: pointer; transition: all 0.15s;`;
const Dropzone = styled.div`border: 2px dashed ${p => (p.$active ? BROWN : '#d8d8d8')}; background: ${p => (p.$active ? '#f5ede4' : '#ececec')}; border-radius: 14px; height: 150px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px; cursor: pointer; transition: all 0.2s; &:hover { border-color: ${BROWN}; background: #f5ede4; }`;
...
const SizeCard = styled.button`min-height: 90px; border-radius: 10px; border: 1.5px solid ${p => (p.$active ? BROWN : '#cfcfcf')}; background: ${p => (p.$active ? '#f5ede4' : 'white')}; color: ${p => (p.$active ? BROWN_DARK : '#222')}; font-size: 15px; font-weight: 600; cursor: pointer; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; transition: all 0.15s; padding: 10px; &:hover { border-color: ${BROWN}; }`;
...
const Toggle = styled.button`position: relative; width: 44px; height: 24px; border-radius: 999px; border: none; cursor: pointer; background: ${p => (p.$on ? BROWN : '#d1d5db')}; transition: background 0.2s; & span { position: absolute; top: 2px; left: ${p => (p.$on ? '22px' : '2px')}; width: 20px; height: 20px; border-radius: 50%; background: white; transition: left 0.2s; }`;
