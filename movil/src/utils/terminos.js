/*
 * ============================================================
 * TÉRMINOS Y AVISO DE PRIVACIDAD — terminos.js (móvil)
 * ============================================================
 * El texto legal de la tienda, escrito como DATOS y no como pantalla.
 *
 * TERCERA COPIA, y a propósito. El original está en
 * `frontend/src/utils/terminos.js` y el backend tiene la suya en
 * `backend/src/utils/terminos.js` con solo la versión. Los tres lados no
 * comparten código —son tres paquetes distintos— y el servidor no puede dejar
 * que el cliente le dicte qué versión se aceptó: bastaría con mandar "9.9"
 * para dejar registrado un consentimiento que nunca existió.
 *
 * REGLA: si sube la versión en un lado, súbala en los tres. Manda la del
 * backend, que es la que se escribe en la cuenta.
 *
 * Y la advertencia honesta que trae el original: esto es un texto claro y de
 * buena fe, no un documento revisado por un abogado.
 * ============================================================
 */

export const VERSION_TERMINOS = '1.0';

// Se muestra al pie. Va escrita y no calculada: es la fecha en que se redactó
// este texto, no la de hoy.
export const FECHA_TERMINOS = '5 de agosto de 2026';

/*
 * La tabla de datos: qué se pide, para qué, quién más lo ve y cuánto se guarda.
 * Es el corazón del aviso — un muro de párrafos legales lo salta todo el mundo.
 */
...
const TERCEROS = [
  ...
  'El mapa (OpenStreetMap y CARTO): cuando busca una dirección, se le manda a OpenStreetMap el texto que escribió para encontrar el punto, y las calles del mapa se descargan de CARTO. Nunca su nombre ni su teléfono.',
...
/*
 * El documento completo. Los bloques van tipados para que la pantalla sea un
 * solo `switch` en vez de un archivo con el texto incrustado a mano.
 */
    ...
    bloques: [
      {
        ...
        puntos: [
          ...
          'Que borremos su cuenta y sus datos: escribiéndonos por WhatsApp desde la tienda. Le respondemos dentro de los 15 días siguientes.',
        ],
      },
      {
        tipo: 'nota',
